...
- C言語による組み込みアプリケーションの開発を5年間経験しました。携帯電話向けWebブラウザの開発が主です。また、C++でWebKitのカスタマイズも経験しました。
- JavaによるAndroidアプリケーション開発を経験しました。
- AndroidやモバイルアプリためのSDK開発を経験しました。WebViewベースでゲームのためのアプリ開発のSDK、cordovaベースでプリンタアプリ開発のためのSDKも経験しています。
- スタートアップ企業でフルスタックにWebアプリケーションの開発を経験してます。バックエンドのAPIサーバーとフロントエンドのSPAの開発を同時に行っていました。サーバーサイドは主にPython、フロントエンドはNext.jsやAngularです。
- Electronによるデスクトップアプリ開発もやっておりました
- プロジェクトマネージャーやスクラムマスターとしてプロジェクトの推進する経験があります。スクラムマスターとしてもスクラムブックを元に、スクラムイベントのファシリテーションやアジリティ向上のための改善をお粉ていました。
...
スクラムマスターとしては、チームにスクラムの定着として、
各種スクラムイベントの主催、タスクやベロシティの管理、
インセプションデッキの作成、そして他チームとの連携を行いました。

また、プロダクトオーナーと顧客インタビューに同行し直接インタビューを行ったり、
社内でブレインストーミングを行い、リーンキャンバスを何度も書く
など、顧客価値を出すプロダクトの企画も行いました。

途中でアウトプットを優先させるため、スクラムマスタを採用し、自分は開発に専念しました。
ただスクラムマスターの経験は無駄にはなりませんでした。
